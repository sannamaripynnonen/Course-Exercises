"""
TIE-02100 Johdatus ohjelmointiin, kevät 2019
...
def read_friendship_network(filename):
    ...
    # Initializing a variable named "network" which contains all the relevant
    # information concerning friendships when used later in the program.
    # Key is a person's name and the value is a list containing the person's
    # friends.
    ...
    for name in [name1, name2]:
        if not name.isalpha():
            print(f"Error: '{name}' is not a valid name: "
                  f"friendship not added.")
            return

    # Implementing a sanity check to make sure no one can be added as his or
    # her own friend.
    if name1 == name2:
        return

    # Adding information about <name1> being <name2>'s friend and <name2>
    # being <name1>'s friend into the parameter data structure <network>.
...
def print_command(network, namelist):
    ...
    # Prints the information of all friendships in alphabetical order.
    else:
        for name in sorted(network):
            print(name)
            friends = network[name]
            for names in sorted(friends):
                print("-", names)
...
def friends_command(network, namelist):
    ...
    # Printing name's friends in alphabetical order
    for names in sorted(network[name]):
        print(names)
...
def common_friends_command(network, namelist):
    ...
    # Making lists <friends_name1> and <friends_name2> of
    # <name1>'s and <name2>'s friends. Going through both lists and
    # making a list <common_friends> with friends that appear in both lists.
    else:
        friends_name1 = network[name1]
        friends_name2 = network[name2]
        common_friends = []

        for name in friends_name1:
            if name in friends_name2:
                common_friends.append(name)
